...
<h2>主要功能</h2>
<ul><li>本页面的主脚本是经过作者安全测试过的脚本弹幕，可以放心使用，不用担心被系统屏蔽封禁发言，作者把火力全开搜寻🔥和自动发弹幕💥的二者功能分离开来，虽然操作上比下面的二合一脚本繁琐了些，但可以保证大家账号的安全，不过需要两个账号来操作(一个用搜寻，一个发弹幕)；</li>
<li>需要斗鱼全自动搜索火力全开房间+自动发弹幕抢鱼丸红包二合一脚本功能的水友们，请下载本段末尾的脚本链接，之所以多此一举，是让大家充分了解使用这个二合一全自动脚本的使用风险。二合一脚本可以满足一些每天看斗鱼时间较短(一般2~3h以内的用户)，喜欢冒险的人或纯粹的懒人，特别需要注意图标🌐火力全开搜寻跳转次数不要超过100次(超过100次建议大家停止使用，或换号使用，<strong><a style="color:red">切记！切记!</a></strong>)，二合一脚本和此页面的安全脚本使用方法基本一致，不另做说明，两个脚本同步更新，请<strong><a style="color:red; text-decoration:none" href="https://raw.githubusercontent.com/wolf-scream/FirePowerSeek/master/AutoSeek_SendMsg.js" target="_blank">这里下载</a></strong>；</li></ul>
<hr>
<h2><a style=" color:red; text-decoration:none; background-color:yellow">必知内容</a></h2>
<p>本脚所包含的所有功能：自动搜索火力全开房间+自动随机发送云端弹幕+皇帝弹幕特效+不绑定手机发弹幕+房间自动签到+房间已签到排行+房间在线人数+当日跳转次数+平台礼物数量+主播信用值+主播开播时长+画质调整(用户自行开启)+是否自动播放(记忆上次操作)+关闭滚屏弹幕(记忆上次操作)</p>
<ol>
    <li>如果是为了使用帝王弹幕特效和不绑手机发弹幕，查看等功能，那将复选框点到⛔-火力停止状态,等待组件初始化就好了，其他的都不理会，按以往您斗鱼习惯操作就好。以下内容是其他功能的详细介绍，<strong>懒人可以不看！</strong></li>
    <li>如果是为了抢丸子或红包，那一定要记住要<strong><a style="color:red">用小号，用小号，用小号</a></strong>重要说三遍，小号可以不绑定手机但要用任意账号登陆登录（比如用QQ、微信或微博注册就可以，非登陆状态火力全开是不出现），保持在自动搜索状态🔥，来搜索符合筛选规则的房间（注意：这个小号尽量不要发弹幕，只做火力全开房间搜寻和跳转使用，仅跳转的话上千次都没问题，有丧心病狂大牛测试一天跳转3000多次，没毛病）。通过小号找到筛选合适的网址拷贝下来。然后用自己的主账号访问刚刚复制的网址，状态保持在💥弹幕轰炸功能，即自动发弹幕抢丸子红包，二合一脚本则省略此步骤。脚本可以自动检测有无火力全开而发送或停止弹幕，主账号可以开两个页面抢丸子，不建议超过三个，再多了会被斗鱼系统识别而被屏蔽发言（亲测），因为要登陆两个甚至多个账号建议用不同浏览器登陆，否则一个浏览器开页面多了容易引起浏览器崩溃；</li>
    <li>被斗鱼屏蔽发言的场景，一般满足频繁房间跳转+频繁发弹幕（二者兼备才行）则搜索跳转超过100次后就很有可能被斗鱼屏蔽弹幕（一般不超过200次）。针对这个限制我们可以想办法规避，一是就是本脚本所做的，把自动搜索和自动发弹幕二者功能分离，这样避免了被斗鱼封禁的可能性；二是针对自动跳转的时间间隔，每个新房间都停留个几分钟再跳转，这样就不属于短时间频繁房间跳转，但坏处显而易见，太浪费时间了，所以这个方法弃用了；</li>
...
<h2>脚本说明</h2>
    ...
    <p>本脚本的使用方法特别简单，因为可供操作的区域都集中在房间标题右侧附近，如图所示；</p>
<ul>
...
<ol>    
    ...
    <li>云弹幕功能已经集成了，这回大家可以解放双手，不用再为发啥子弹幕而愁了，自动从云端抽取符合房间类别的热门弹幕，就是这么牛；新增滚屏弹幕和自动播放开关记忆功能，满足大家不同场景的需求</li>
...
<ul>
    ...
    <li>问题No.3 浏览器经常崩溃，这种情况是最为常见的，基本上是由于浏览器内存不足造成的，即用户打开的页面太多了，所以建议您用多个不同浏览器满足多个页面打开的需求，不建议用一个浏览器打开过多的页面，只要是chrome内核的浏览器就可以，如360、搜狗，也都能装油猴的;</li>
    <li>问题No.4 什么是被斗鱼系统屏蔽发言？ 简单的说就是你发出的弹幕，无论在聊天区、滚屏区，都不显示，PC客户端显示但是假象，即房间其他用户看不到你发言，你的弹幕被斗鱼系统拦截了，因此你也无法参与弹幕抽奖活动，这是斗鱼限制用户使用第三方弹幕插件采取的相应措施;</li>
<!--     <li>问题No.5 用户自行开启方法如何启用？ 由于自行开启的三个方法使用的人数很少，所以默认关闭状态，用户可依据自己的需求有选择开启，方法如下图所示；</li>
     <table><tr><td><img src="https://raw.githubusercontent.com/wolf-scream/FirePowerSeek/master/pics/userSelect.jpg" width="100%"></td></tr></table> -->
</ul><hr>    
...
<p><strong>在线人数参考qianjiachun大佬的API接口实现的，调整画质参考wah0713大神的方法实现，在此竭诚拜谢二位的指引与贡献。除了这两处参考，其他均为作者原创；</strong></p>
<hr>
<h2>写在最后</h2>
<ul>
